...
const SimplePeer =
  SimplePeerImport && SimplePeerImport.default
    ? SimplePeerImport.default
    : globalThis.SimplePeer || SimplePeerImport;
...
export class WebtorrentProvider extends Observable {
  ...
  async connect(opts = {}) {
    ...
    this.trackerConnections = this.trackers.map(
      (url) =>
        new TrackerConnection(url, {
          infoHash: this.infoHash,
          peerId: this.peerId,
          numwant: this.numwant,
          WebSocket: opts.WebSocket,
          createOffers: (tracker) => this.createOffers(tracker),
          onOffer: (peerId, offerId, offer, tracker) =>
            this.receiveOffer(peerId, offerId, offer, tracker),
  ...
  createPeer(remotePeerId, initiator) {
    ...
    peer.on("signal", (signal) =>
      this.emitDebug({ type: "peer-signal", signalType: signal.type, initiator }),
    );
    peer.on("_debug", (message) =>
      this.emitDebug({ type: "simple-peer-debug", message, initiator }),
    );
